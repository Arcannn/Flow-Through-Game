...
public class MapGen : MonoBehaviour
{
  ...
  // Game objects listed above - parameters for the ceiling/floor and then further a duplicate for the ceiling/floor in the form of ceiling1/floor1 -These specific GameObjects allow the map to keep generating moving forward.
  ...
  void Start()
  {
    Object1 = GenerateObject(player1.transform.position.x + 40);
    Object2 = GenerateObject(Object1.transform.position.x);
    Object3 = GenerateObject(Object2.transform.position.x);
    Object4 = GenerateObject(Object3.transform.position.x);
    // Object1 is the intial object that is spawned +20 away from the player on the x axis. Object1 will then be followed +10 by the object after it and so on.
  }
  GameObject GenerateObject(float referenceX) {
    GameObject Object = GameObject.Instantiate(GenObject);
  ...
  // New Object generation functiona:
  void GenerateNewObj() {
    if (player1.transform.position.x > Object2.transform.position.x) {
      ...
      if(!(temporaryObject.activeSelf)){
        temporaryObject.SetActive(true);
      }
      SetTransform(temporaryObject, Object3.transform.position.x);
      Object4 = temporaryObject;
